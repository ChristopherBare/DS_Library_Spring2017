...
/**
 * LinkedBinarySearchTree class based on implementation in Lewis & Chase
 * textbook Note that this is a very basic search tree that does NOTHING to
...
public class LinkedBinarySearchTree<T> extends LinkedBinaryTree implements BinarySearchTreeADT<T> {
     ...
     * found. Throws an exception if targetElement isn't comparable or if the
     * element isn't found in a tree. Assume that the contains() method has been
     * called first to ensure the element is in the tree
     *
    ...
    /**
     * Private helper method to recursively search sub-trees for element removal
     *
    ...
    /**
     * Remove all occurrences of a target element from the tree Throw an
     * exception if element is not comparable, or if element is not found
     *
     * @param targetElement the element to be searched for
     * @throws ElementNotFoundException
     * @throws NonComparableElementException
    ...
    /**
     * private helper method to move a node up in the tree when a node is
     * removed
     *
    ...
    public T removeMax() throws EmptyCollectionException {
        ...
        int count = 0;
        if (isEmpty()) {
            throw new EmptyCollectionException("Empty binary tree");
        }
        else {
            if (root.getRightChild() == null) {
                result = (T) root.getElement();
                ...
                BTNode<T> parent = root;
                BTNode<T> current = root.getRightChild();
    ...
    /**
     * A method that does a q-order traversal of the tree, creating a list of
     * elements from the root down, with empty nodes specified by null
     *
